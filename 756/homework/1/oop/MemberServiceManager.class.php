...
require 'PreparedStatementSetter.class.php';
...
class MemberServiceManager
{
     ...
     *
     */
    const DELETE_USER_ROLE_SQL        = 'DELETE FROM user_role WHERE username = ?';
    ...
    {
        return new SQLite3( MEMBER_DB );
    }

    public function createUser( MemberForm $user )
    {
        $stmt = $this->getDB()->prepare( self::NEW_USER_SQL );
        PreparedStatementSetter::setValuesAndExecute( function( SQLite3Stmt &$ps ) use ( $user ) {
    ...
    public function updateUser( Member $user )
    {
        ...
        PreparedStatementSetter::setValuesAndExecute(function( SQLite3Stmt &$ps ) use ( $user ) {
            $ps->bindValue( 1, $user->isEnabled() );
            $ps->bindValue( 2, $user->getFirst_name() );
            $ps->bindValue( 3, $user->getLast_name() );
    ...
    public function deleteUser( $username )
    {
        try
        {
            if ( $this->checkForAdminRights( $this->loadShabaUserByUsername( $username ) ) )
                return;
    ...
    private function insertUserAuthorities( Member $user, $auth )
    {
        $stmt = $this->getDB()->prepare( self::NEW_USER_ROLE_SQL );
        PreparedStatementSetter::setValuesAndExecute( function( SQLite3Stmt &$ps ) use ( $user, $auth ) {
            $ps->bindValue( 1, $user->getUsername(), SQLITE3_TEXT );
    ...
    private function deleteUserAuthorities( $username )
    {
        ...
        PreparedStatementSetter::setValuesAndExecute( function( SQLite3Stmt &$ps ) use ( $username ) {
    ...
    public function changePassword( $oldPassword,  $newPassword, $username = null )
    {
        ...
        if ( $currentUser->getUsername() == $username &&
             $currentUser->getPassword() != $oldPassword )
        {
            throw new Exception( 'Old password does not match.' );
        }
        else if ( !in_array( $currentUser->getAuthorities(), 'ROLE_ADMIN' ) )
        {
            throw new Exception(
        ...
        $stmt = $this->getDB()->prepare( self::UPDATE_PASSWORD_SQL );
            PreparedStatementSetter::setValuesAndExecute( function( SQLite3Stmt &$ps ) use ( $newPassword, $username ) {
    ...
    {
        return loadMembersByUsername( $_SESSION[USERNAME] );
    }

    private function checkForAdminRights( Member $user )
    {
        return $user == null && $user->isEnabled() && in_array( 'ROLE_ADMIN', $user->getAuthorities() );
    }

    private function failIfNotAdmin( $message = 'A non-administrator cannot do that!' )
    {
        $user = $this->getCurrentUser();

        if ( $user == null || !$user->isEnabled() || !in_array( 'ROLE_ADMIN', $user->getAuthorities() ) )
        {
     ...
     * Member objects. There should normally only be one matching user.
     */
    protected function loadMembersByUsername( $username )
    {
        ...
        return self::extractData( $result );
    }
    ...
    private static function mapRow( $rs )
    {
        ...
        $user = new Member( $rs['username'], $rs['password'],
                $rs['first_name'], $rs['last_name'], $rs['email'],
                $rs['date_joined'], $rs['last_online'], $rs['enabled'],
                $roles );
    ...
    public static function extractData( $rs )
    {
        ...
        while ($res = $rs->fetchArray( SQLITE3_ASSOC ) )
        {
            ...
            if ( array_key_exists( $user->getUsername(), $results ) )
            {
                ...
                $auths = array_merge( $inUser->getAuthorities(), $user->getAuthorities() );
            } else
            {
