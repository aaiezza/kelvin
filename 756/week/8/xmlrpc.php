...
{
    return new xmlrpcresp( new xmlrpcval( 'Hello Math Fans!', 'string' ) );
}
...
function calcCircle( $params )
{
    ...
    return ( $radius > 0 ) ? new xmlrpcresp( new xmlrpcval( pi * $radius * $radius, 'double' ) ) : new xmlrpcresp( 
            new xmlrpcval( 'Error: radius is required' ) );
}

function calcRectangle( $params )
{
    // parse our params
    $width = $params->getParam( 0 )->scalarval();
    $height = $params->getParam( 1 )->scalarval();
    
    return ( $width + $height > 0 ) ? new xmlrpcresp( new xmlrpcval( $width * $height, 'double' ) ) : new xmlrpcresp( 
 ...
 */
$helloWorld_sig = array ( array ( $xmlrpcArray ) );
$calcCircle_sig = array ( array ( $xmlrpcDouble, $xmlrpcDouble ) );
$calcRectangle_sig = array ( array ( $xmlrpcDouble, $xmlrpcDouble, $xmlrpcDouble ) );
