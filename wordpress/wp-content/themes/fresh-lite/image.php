							...
							<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
								<header class="entry-header">
									...
									<div class="entry-meta">
										...
										printf( __( 'Published', 'fresh-lite' ) . ' <span class="entry-date"><time class="entry-date" datetime="%1$s">%2$s</time></span> - ' . __( 'size', 'fresh-lite' ) . ': <a href="%3$s" title="Link to full-size image">%4$s &times; %5$s</a> ' . __( 'in', 'fresh-lite' ) . ' <a href="%6$s" title="Return to %7$s" rel="gallery">%7$s</a>',
										...
										?>
										<?php edit_post_link( __( 'Edit', 'fresh-lite' ), '<span class="sep"> | </span> <span class="edit-link">', '</span>' ); ?>
									</div>
									<!-- .entry-meta -->

									<nav id="image-navigation" class="row-fluid">
										<div class="span6">
											<div class="previous-image"><?php previous_image_link( false, '&larr; ' . __( 'Previous', 'fresh-lite' ) ); ?></div>
										</div>
										<div class="span6">
											<div class="next-image alignright"><?php next_image_link( false, __( 'Next', 'fresh-lite' ) . ' &rarr;' ); ?></div>
										</div>
									</nav>
									<!-- #image-navigation -->
								</header>
								...
								<div class="entry-content">
									...
									<?php the_content(); ?>
									<?php wp_link_pages( array( 'before' => '<div class="page-links">' . __( 'Pages:', 'fresh-lite' ), 'after' => '</div>' ) ); ?>
								...
								<footer class="entry-meta">
									<?php if ( comments_open() && pings_open() ) : // Comments and trackbacks open ?>
										<?php printf( '<a class="comment-link" href="#respond" title="Post a comment">' . __( 'Post a comment', 'fresh-lite' ) . '</a> ' . __( 'or leave a trackback', 'fresh-lite' ) . ': <a class="trackback-link" href="%s" title="Trackback URL for your post" rel="trackback">' . __( 'Trackback URL', 'fresh-lite' ) . '</a>.', get_trackback_url() ); ?>
									<?php elseif ( !comments_open() && pings_open() ) : // Only trackbacks open ?>
										<?php printf( __( 'Comments are closed, but you can leave a trackback:', 'fresh-lite' ) . ' <a class="trackback-link" href="%s" title="Trackback URL for your post" rel="trackback">' . __( 'Trackback URL', 'fresh-lite' ) . '</a>.', get_trackback_url() ); ?>
									<?php
									elseif ( comments_open() && !pings_open() ) : // Only comments open
										?>
										<?php _e( 'Trackbacks are closed, but you can', 'fresh-lite' ) . ' <a class="comment-link" href="#respond" title="Post a comment">' . __( 'post a comment', 'fresh-lite' ) . '</a>.'; ?>
									<?php
									elseif ( !comments_open() && !pings_open() ) : // Comments and trackbacks closed
										?>
										<?php _e( 'Both comments and trackbacks are currently closed.', 'fresh-lite' ); ?>
									<?php endif; ?>
									<?php edit_post_link( __( 'Edit', 'fresh-lite' ), ' <span class="edit-link">', '</span>' ); ?>
								</footer>
								<!-- .entry-meta -->
