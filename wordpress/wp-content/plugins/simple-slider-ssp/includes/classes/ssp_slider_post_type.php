...
class SSP_SLIDER_POST_TYPE {
	...
	function shortcode_meta_box( $post ) {
		...
		if ( get_post_status( $slider_id ) !== 'publish' ) {
			...
			echo __( '<p>Please click on the Create Slider button to get the slider shortcode</p>', 'ssp' );
		...
		$shortcode = sprintf( "[%s id='%s' name='%s' size='full']", SLIDER_PLUGIN_SLIDER_SHORTCODE, $slider_id, $slider_title );
	...
	function options_meta_box( $post ) {
		...
		$skins[] = array(
			'name' => __( 'Default Image Skin', 'ssp' ),
			'path' => 'default',
			'description' => ''
		);
